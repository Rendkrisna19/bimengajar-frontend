    ...
    <div className="flex flex-col gap-6">
                ...
                {kegiatan.length > 0 ? (
                  kegiatan.map((item, index) => (
                    <tr key={item.id} className="hover:bg-blue-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${item.status === 'Terlaksana' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                          {item.status}
                        </span>
                      </td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Kegiatan' : 'Tambah Kegiatan'}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Judul Kegiatan <span className="text-red-500">*</span></label>
            <input
              type="text"
              name="judul"
              ...
              required
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#003366]/20 focus:border-[#003366] transition-colors"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Tgl Mulai <span className="text-red-500">*</span></label>
              <input
                type="date"
                name="tanggal_mulai"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Tgl Selesai (Opsional)</label>
              <input
                type="date"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Jenis Kegiatan</label>
              <select
                name="jenis_kegiatan"
